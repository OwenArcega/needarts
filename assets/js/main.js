let size = 8;
let key = "ce612d76-3dd4-47e7-a7cf-fdab149106f8"
...
let harvardCollection = (size, keyword, centuryId, techniqueId, classificationId) =>{
    fetch (`https://api.harvardartmuseums.org/object?sort=accessionyear&size=${size}&q=${keyword}&technique=${techniqueId}&century=${centuryId}&classification=${classificationId}&apikey=${key}`)
...
fetch(`https://api.harvardartmuseums.org/century?size=100&sort=name&apikey=${key}`)
    .then((response) => response.json())
    .then((century) => {
        writeCenturyFilter(century);
    })
...
fetch(`https://api.harvardartmuseums.org/classification?size=100&sort=name&apikey=${key}`)
    .then((response) => response.json())
    .then((classification) => {
        writeClassificationFilter(classification);
    })
...
fetch(`https://api.harvardartmuseums.org/technique?size=318&sort=name&apikey=${key}`)
    .then((response) => response.json())
    .then((technique) => {
        writeTechniqueMediumFilter(technique);
    })